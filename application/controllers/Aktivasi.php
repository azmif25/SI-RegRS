...
class aktivasi extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $total = $this->m_aktivasi->get_total();

        if ($total > 0) {
            // get current page records
            $params['results'] = $this->m_aktivasi->get_current_page($limit_page, $page * $limit_page);

            $config['base_url'] = base_url() . 'aktivasi/index';
        ...
        // $data['recs'] = $this->m_aktivasi->inqaktivasi();

        //load template
        $this->load->view('template/t_header');
        $this->load->view('template/t_sidebar');
        $this->load->view('content/aktivasi/v_idxaktivasi', $params);
        $this->load->view('template/t_footer');
    }
    public function search()
    {
        $keyword = $this->input->post('keyword');
        $data['results'] = $this->m_aktivasi->get_aktivasi_keyword($keyword);

        //load template
        $this->load->view('template/t_header');
        $this->load->view('template/t_sidebar');
        $this->load->view('content/aktivasi/v_idxaktivasi', $data);
        $this->load->view('template/t_footer');
    }
    public function showed()
    {
        $keyword = $this->input->post('tanggal');
        $data['results'] = $this->m_aktivasi->get_aktivasi_keywordsh($keyword);
    ...
    public function rekap()
    {
        ...
        $limit_page = 6;
        $page = ($this->uri->segment(3)) ? ($this->uri->segment(3) - 1) : 0;
        $total = $this->m_aktivasi->get_rtotal();

        if ($total > 0) {
            // get current page records
            $params['results'] = $this->m_aktivasi->get_current_rpage($limit_page, $page * $limit_page);

            $config['base_url'] = base_url() . 'aktivasi/rekap';
            $config['total_rows'] = $total;
            $config['per_page'] = $limit_page;
            $config['uri_segment'] = 3;
        ...
        // $data['recs'] = $this->m_aktivasi->inqraktivasi();

        //load template
        $this->load->view('template/t_header');
        $this->load->view('template/t_sidebar');
        $this->load->view('content/aktivasi/v_idxraktivasi', $params);
        $this->load->view('template/t_footer');
    }
    ...
    public function cetak()
    {
        ...
        $data['recs'] = $this->m_aktivasi->inqraktivasi($daritgl, $ketgl);

        //load template
        $this->load->view('content/aktivasi/v_ctkaktivasi', $data);
    }
    ...
    {
        if ($this->input->post('nik') == $this->input->post('nik_confirm')) {
            $id_aktivasi = $this->input->post('id_aktivasi');
            $id_admin = $this->input->post('id_admin');
            $tgl_aktivasi = $this->input->post('tgl_aktivasi');

            $this->db->trans_start();
            $this->m_aktivasi->updaktivasi($id_aktivasi, $id_admin, $tgl_aktivasi);
            $this->db->trans_complete();

            redirect('aktivasi');
        } else {
            echo "<script>alert('NIK Pasien yang dimasukan salah!');</script>";
